...
export const Banner: React.FC = () => {
  ...
  let cssFromBackend = "";
  // → 2) Extrae las clases del NAV que guardaste en BannerSection2:
  const backendNavClasses = useMemo(() => {
    const sec = sections.find((s) => s.key === "BannerSection2");
    if (!sec) return "";
    const m = sec.html.match(/<nav[^>]*class="([^"]*)"/);
    return m ? m[1] : "";
  }, [sections]);
  ...
  return (
    <>
      ...
      >
        ...
        {renderSection(sections, "BannerSection1") || (
        ...
        <nav
          /* 3) reemplaza solo el envoltorio */
          className={
            backendNavClasses ||
